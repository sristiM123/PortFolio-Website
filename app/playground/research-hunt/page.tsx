...
export default function ResearchHunt() {
    ...
    function choose(opt: string) {
        setPicked(opt);
        if (opt === "IEEE Xplore") {
            setMsg("✅ Exactly. Opening Research…");
            setTimeout(() => router.push("/research"), 700);
        } else {
            setMsg("ℹ️ Close. The MAKE paper is on IEEE Xplore.");
        }
    }

    const opts = ["IEEE Xplore", "Random Blog", "Social Media"];
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50 px-4 sm:px-6 lg:px-8 py-12">
            <div className="mx-auto max-w-xl rounded-2xl border border-amber-100 bg-white/90 p-6 shadow-sm">
                <h1 className="text-3xl font-extrabold text-orange-900">Research Hunt</h1>
                <p className="mt-2 text-neutral-700">Where is my “MAKE” paper published?</p>
